...
import sys
import os
def post_jobs(bv, fwd, src, dst, pktlen, full_phy, pwr, message, duration):
    directory = '/home/burhanuddin/Desktop/osf/tools/dcube'
    if bv == 1:
        command = f"./dcube.sh -POST TARGET=nrf52840 -e osf -n BV -d 'bit voting {message} {phy} power {pwr} jamming level none length {pktlen} actual dissemination  nulltb d{dst}f{fwd}s{src}' -p 6907 --s=1 -nopatch --dur={duration} --data={pktlen - 9} --period=250 -m DST={dst} FWD={fwd} SRC={src} TESTBED=nulltb PERIOD=1000 CHN=0 LOGGING=1 GPIO=1 LEDS=1 NTX=6 NSLOTS=6 PWR={pwr} PROTO=OSF_PROTO_BCAST PHY={full_phy} BV={bv} LENGTH={pktlen - 9} DCUBE=1"
    else:
        command = f"./dcube.sh -POST TARGET=nrf52840 -e osf -n BV -d 'No bit voting {message} {phy} power {pwr} jamming level none length {pktlen} actual dissemination nulltb d{dst}f{fwd}s{src}' -p 6907 --s=1 -nopatch --dur={duration} --data={pktlen - 7} --period=250 -m DST={dst} FWD={fwd} SRC={src} TESTBED=nulltb PERIOD=1000 CHN=0 LOGGING=1 GPIO=1 LEDS=1 NTX=6 NSLOTS=6 PWR={pwr} PROTO=OSF_PROTO_BCAST PHY={full_phy} BV={bv} LENGTH={pktlen - 7} DCUBE=1"      
...
def post_jobs_templab(bv, fwd, src, dst, full_phy, pktlen, templab_file, pwr, message, duration):
    ...
    if templab_file:
        if bv == 1:
            command = f"./dcube.sh -POST TARGET=nrf52840 -e osf -n BV -d 'bit voting {message} {phy}  power {pwr} jamming level one length {pktlen} actual dissemination templab nulltb d{dst}f{fwd}s{src}' -p 6907 --s=1 -nopatch --dur={duration} --data={pktlen - 9} --period=250 -m DST={dst} FWD={fwd} SRC={src} TESTBED=nulltb PERIOD=1000 CHN=0 LOGGING=1 GPIO=1 LEDS=1 NTX=6 NSLOTS=6 PWR={pwr} PROTO=OSF_PROTO_BCAST PHY={full_phy} BV={bv} LENGTH={pktlen - 9} DCUBE=1 -templab {templab_file}"
        else:
            command = f"./dcube.sh -POST TARGET=nrf52840 -e osf -n BV -d 'no bit voting {message} {phy} power {pwr} jamming level none length {pktlen} actual dissemination templab nulltb d{dst}f{fwd}s{src}' -p 6907 --s=1 -nopatch --dur={duration} --data={pktlen - 7} --period=250 -m DST={dst} FWD={fwd} SRC={src} TESTBED=nulltb PERIOD=1000 CHN=0 LOGGING=1 GPIO=1 LEDS=1 NTX=6 NSLOTS=6 PWR={pwr} PROTO=OSF_PROTO_BCAST PHY={full_phy} BV={bv} LENGTH={pktlen - 7} DCUBE=1 -templab {templab_file}"
...
if __name__ == '__main__':
    ...
    phys=[]
    print(args.packetlength)
    if args.packetlength == 125:
        print("Packet length is 128")
        phys = ['IEEE']
    elif args.packetlength == 255:
        phys = ['BLE_1M', 'BLE_2M', 'BLE_125K', 'BLE_500K']
    print(len(phys))
    for phy in phys:
        full_phy = 'PHY_'+phy
        if args.templab:
            ...
            post_jobs(bv, fwd, src, dst, args.packetlength, full_phy, args.power, args.message, args.duration)